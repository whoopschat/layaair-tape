...
(function(window,document,Laya){
	...
	var Arith=laya.maths.Arith,AtlasResourceManager=laya.webgl.atlas.AtlasResourceManager,Bitmap=laya.resource.Bitmap;
	var Browser=laya.utils.Browser,Byte=laya.utils.Byte,Canvas=BK.Canvas,Color$1=laya.utils.Color,Event=laya.events.Event;
	var EventDispatcher=laya.events.EventDispatcher,Font=laya.display.css.Font,HTMLCanvas=laya.resource.HTMLCanvas;
	var HTMLImage=laya.resource.HTMLImage,Handler=laya.utils.Handler,HitArea=laya.utils.HitArea,Input=laya.display.Input;
	var Loader=laya.net.Loader,LocalStorage=laya.net.LocalStorage,Matrix=laya.maths.Matrix,MouseManager=laya.events.MouseManager;
	...
	var RenderSprite=laya.renders.RenderSprite,RenderState2D=laya.webgl.utils.RenderState2D,RenderTarget2D=laya.webgl.resource.RenderTarget2D;
	var RunDriver=laya.utils.RunDriver,Socket=laya.net.Socket,SoundChannel=laya.media.SoundChannel,SoundManager=laya.media.SoundManager;
	...
	var TouchManager=laya.events.TouchManager,URL=laya.net.URL,Utils=laya.utils.Utils,WebGL=laya.webgl.WebGL;
	var WebGLCanvas=laya.webgl.resource.WebGLCanvas,WebGLCharImage=laya.webgl.resource.WebGLCharImage,WebGLContext=laya.webgl.WebGLContext;
...
var BKColor=(function(){
	...
	BKColor.getBKColor=function(color){
		var color=Color$1.create(color);
		if (!color["colorO"]){
			var colorO;
			colorO={};
			var arr;
...
var LoaderAdpt=(function(){
	...
	var __proto=LoaderAdpt.prototype;
	...
	*/
	__proto._loadImage=function(url){
		...
		bmp=new BKBitmap();
		bmp.src=url;
		Laya.timer.frameOnce(2,this,this.onLoaded,[bmp]);
	}
	...
	*/
	__proto._loadSound=function(url){
		Laya.timer.frameOnce(2,this,this.onLoaded,[{}]);
	}
...
var RenderContextAdpt=(function(){
	...
	RenderContextAdpt.__init=function(){
		...
		FunctionReplaceTool.replaceClassFuns(RenderContext,RenderContextAdpt,["clear","flush"]);
	}
...
var LayaBKAdpter=(function(){
	...
	LayaBKAdpter._preInits=function(){
		...
		LayaBKAdpter.window.location={"protocol":"file:","pathname":"GameRes://" };
		...
		WebGLCharImageAdpt._init();
		WebGLImageAdpt.__init();
...
var TextTool=(function(){
	...
	TextTool.getColorObj=function(colorStr){
		var color=Color$1.create(colorStr);
		var rst;
		rst={};
		rst.a=1;
...
*@author ww
*/
//class BKLaya.tools.TextureUtils
var TextureUtils=(function(){
	function TextureUtils(){}
	__class(TextureUtils,'BKLaya.tools.TextureUtils');
	TextureUtils.createTextureByBuffer=function(buff,width,height){
		var bmp;
		bmp=new BKBitmap();
		bmp.initByBuffer(buff,width,height);
		var tex;
		tex=new Texture(bmp);
		return tex;
	}
...
*@author ww
*/
//class BKLaya.adptclass.BKBitmap extends laya.resource.Bitmap
var BKBitmap=(function(_super){
	function BKBitmap(){
		BKBitmap.__super.call(this);
	}

	__class(BKBitmap,'BKLaya.adptclass.BKBitmap',_super);
	var __proto=BKBitmap.prototype;
	__proto.initByBuffer=function(buff,width,height){
		this._source=new BK.Texture(buff,width,height);
		this._w=width;
		this._h=height;
	}

	__proto.dispose=function(){
		console.log("BKBitmap dispose:",this.url);
		if (this._source)
	...
	__getset(0,__proto,'src',function(){
		...
		this._url=value;
		try {
			this._source=BK.Texture.createOrphanTexture(value);
			var size=this._source.size;
			this._w=size.width;
			this._h=size.height;
			this._source=value;
		}
		catch (e){
			console.log("Fail to create texture:",value);
		}
	});
...
var WebGLCharImageAdpt=(function(_super){
	...
	WebGLCharImageAdpt._init=function(){
		...
		FunctionReplaceTool.replaceClassFuns(WebGLCharImage,WebGLCharImageAdpt,replaceFuns);
	}
...
*@author ww
*/
//class BKLaya.adptcodes.WebGLImageAdpt extends laya.resource.HTMLImage
var WebGLImageAdpt=(function(_super){
	function WebGLImageAdpt(){
		/**@private */
		this._format=0;
		/**@private */
		this._mipmap=false;
		...
		/**放大过滤器*/
		this.magFifter=0;
		WebGLImageAdpt.__super.call(this);
	}

	__class(WebGLImageAdpt,'BKLaya.adptcodes.WebGLImageAdpt',_super);
	var __proto=WebGLImageAdpt.prototype;
	...
	WebGLImageAdpt.__init=function(){
		var replaceFuns;
		replaceFuns=["dispose","_createWebGlTexture","disposeResource","clearAtlasSource"];
		FunctionReplaceTool.replaceClassFuns(WebGLImage,WebGLImageAdpt,replaceFuns);
	}

	return WebGLImageAdpt;
})(HTMLImage)
